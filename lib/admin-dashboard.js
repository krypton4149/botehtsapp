...
const { menuCategoryEmoji } = require('./menu-presentation');
...
function buildOrderRows(orders, R) {
  return orders.slice().map((o) => {
      ...
      <td class="addressCell" title="${escapeHtml(o.address)}">${escapeHtml(o.address)}</td>
...
function dashboardStyles() {
  return `<style>
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .nameCell{display:flex;align-items:flex-start;gap:12px}
    .avatar{
    ...
    .page--orders .addressCell{max-width:min(340px, 34vw)}
    ...
    .page--menu .main{padding-top:8px}
    .menuHero{
      position:relative;border-radius:22px;overflow:hidden;margin-bottom:22px;
      border:1px solid rgba(255,186,90,.22);
      background:
        radial-gradient(900px 420px at 12% 0%, rgba(255,138,31,.22), transparent 55%),
        radial-gradient(700px 380px at 92% 20%, rgba(120,130,255,.14), transparent 50%),
        linear-gradient(135deg, rgba(22,24,39,.96), rgba(14,15,22,.98));
      box-shadow:0 20px 50px rgba(0,0,0,.45), inset 0 1px 0 rgba(255,255,255,.06);
    }
    .menuHeroGlow{
      position:absolute;inset:-40% -20% auto -20%;height:120px;
      background:linear-gradient(90deg, transparent, rgba(255,186,90,.12), transparent);
      filter:blur(28px);pointer-events:none;
    }
    .menuHeroInner{position:relative;padding:26px 24px 24px}
    .menuHeroKicker{
      font-size:11px;font-weight:800;letter-spacing:.16em;text-transform:uppercase;
      color:rgba(255,186,90,.88);
    }
    .menuHeroTitle{
      margin-top:10px;font-size:clamp(24px, 3.2vw, 32px);font-weight:820;line-height:1.1;
      letter-spacing:-.03em;color:rgba(255,255,255,.96);
    }
    .menuHeroSub{margin-top:10px;font-size:14px;color:var(--muted);line-height:1.5;max-width:520px}
    .menuHeroChips{display:flex;flex-wrap:wrap;gap:8px;margin-top:16px}
    .menuHeroChip{
      font-size:12px;font-weight:650;padding:8px 14px;border-radius:999px;
    ...
    .menuCat{margin-top:0}
    .menuCat:first-of-type{margin-top:0}
    .menuCatPanel{
      margin-top:22px;padding:4px 4px 18px;border-radius:20px;
      background:linear-gradient(180deg, rgba(255,255,255,.045), rgba(255,255,255,.02));
      border:1px solid rgba(255,255,255,.09);
      box-shadow:var(--shadow2);
    }
    .menuCatPanel:first-of-type{margin-top:4px}
    .menuCatHead{
    ...
    .menuCatIcon{
      ...
      font-size:22px;
      background:linear-gradient(145deg, rgba(255,186,90,.18), rgba(255,138,31,.08));
      border:1px solid rgba(255,186,90,.28);
      box-shadow:0 10px 26px rgba(255,138,31,.12);
    }
    .menuCatTitle{
      font-size:17px;font-weight:820;letter-spacing:-.02em;
      color:rgba(255,255,255,.94);margin:0;padding:0;
    }
    .menuCatHint{font-size:12px;color:var(--muted2);margin-top:4px}
    .menuGrid{
      display:grid;grid-template-columns:repeat(auto-fill,minmax(min(100%,280px),1fr));gap:12px;
      padding:14px 14px 4px;
    }
    .menuItem{
      padding:0;border-radius:16px;overflow:hidden;
      background:linear-gradient(125deg, rgba(255,255,255,.07), rgba(255,255,255,.025));
      border:1px solid rgba(255,255,255,.10);
    ...
    .menuItem:hover{
      ...
      box-shadow:0 16px 36px rgba(0,0,0,.38), 0 0 0 1px rgba(255,186,90,.12);
    }
    .menuItemRow{
      display:flex;align-items:stretch;gap:0;min-height:72px;
    }
    .menuItemAccent{
      width:4px;flex-shrink:0;
      background:linear-gradient(180deg, rgba(255,186,90,.95), rgba(255,138,31,.55));
    }
    .menuItemBody{
      flex:1;padding:14px 16px 14px 14px;display:flex;flex-direction:column;justify-content:center;gap:6px;min-width:0;
    }
    .menuItemTop{display:flex;align-items:center;justify-content:space-between;gap:10px}
    .menuItemId{
      font-family:ui-monospace,SFMono-Regular,Menlo,monospace;
      font-size:12px;font-weight:800;letter-spacing:.1em;
      color:rgba(255,214,140,.98);
      padding:4px 10px;border-radius:10px;
      background:rgba(0,0,0,.28);border:1px solid rgba(255,186,90,.22);
    }
    .menuItemPrice{
      font-size:14px;font-weight:800;color:rgba(255,255,255,.92);
      padding:6px 12px;border-radius:999px;flex-shrink:0;
      background:rgba(255,138,31,.14);border:1px solid rgba(255,138,31,.28);
      margin-top:0;
    }
    .menuItemName{
      font-size:15px;font-weight:720;line-height:1.38;margin-top:0;
    ...
    @media (max-width: 820px){
      ...
      .addressCell{max-width:min(220px, 52vw)}
    }
  </style>`;
}
...
function menuHeroHtml(R) {
  const tag = R.tagline ? escapeHtml(R.tagline) : 'Order with quick codes on WhatsApp';
  const brand = escapeHtml(R.chatBrand || R.name || 'Restaurant');
  return `<div class="menuHero" role="region" aria-label="Menu overview">
    <div class="menuHeroGlow" aria-hidden="true"></div>
    <div class="menuHeroInner">
      <p class="menuHeroKicker">Live menu</p>
      <h2 class="menuHeroTitle">${brand}</h2>
      <p class="menuHeroSub">${tag}</p>
      <div class="menuHeroChips">
        <span class="menuHeroChip">${escapeHtml(R.hoursShort)}</span>
        <span class="menuHeroChip">Min order · ${escapeHtml(R.currency)}${escapeHtml(String(R.min_order))}</span>
        <span class="menuHeroChip">${escapeHtml(R.areaShort)}</span>
      </div>
    </div>
  </div>`;
}
  ...
  return Object.entries(menuRecord)
    .map(([cat, items]) => {
      ...
      const cards = items
        .map(
          (i) => `<div class="menuItem">
            <div class="menuItemRow">
              <div class="menuItemAccent" aria-hidden="true"></div>
              <div class="menuItemBody">
                <div class="menuItemTop">
                  <span class="menuItemId">${escapeHtml(i.id)}</span>
                  <span class="menuItemPrice">${escapeHtml(R.currency)}${escapeHtml(String(i.price))}</span>
                </div>
        ...
        .join('');
      return `<section class="menuCatPanel" aria-label="${escapeHtml(cat)}">
        <header class="menuCatHead">
          <div class="menuCatIcon" aria-hidden="true">${icon}</div>
          <div>
            <h3 class="menuCatTitle">${escapeHtml(cat)}</h3>
            <div class="menuCatHint">${items.length} item${items.length === 1 ? '' : 's'}</div>
          </div>
        </header>
...
function menuSection(R, menuDb) {
  ...
  return `${menuHeroHtml(R)}
      <section class="section menuSection" aria-label="Menu">
        <div class="sectionHead">
          <div>
            <div class="sectionTitle">Full menu</div>
            <div class="sectionHint">Same dishes & codes customers use in WhatsApp</div>
          </div>
