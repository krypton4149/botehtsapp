...
function dashboardStyles() {
  return `<style>
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .setGrid{display:flex;flex-direction:column;gap:16px;margin-top:8px}
    .setCard{padding:18px 20px}
    .setCard h2{font-size:16px;font-weight:750;margin-bottom:4px}
    .setCardIntro{color:var(--muted2);font-size:12px;margin-bottom:14px;line-height:1.45}
    .setRow{display:flex;flex-wrap:wrap;gap:8px 20px;padding:12px 0;border-bottom:1px solid rgba(255,255,255,.06);align-items:baseline}
    .setRow:last-child{border-bottom:0}
    .setLabel{flex:0 0 150px;color:var(--muted2);font-size:11px;text-transform:uppercase;letter-spacing:.1em}
    .setVal{flex:1;min-width:200px;color:rgba(255,255,255,.86);font-size:14px;word-break:break-word}
...
function settingsSection(R, meta) {
  ...
  const idRow = phoneOk
    ? `<code>${escapeHtml(meta.phoneNumberId)}</code>`
    : '<span class="statusBad">Not set — add <code>PHONE_NUMBER_ID</code> to <code>.env</code></span>';
  const tokRow = meta.hasAccessToken
    ? '<span class="statusOk">Present — <code>ACCESS_TOKEN</code> is set (secret not shown)</span>'
    : '<span class="statusBad">Missing — add <code>ACCESS_TOKEN</code> to <code>.env</code></span>';
  const verifyRow = meta.verifyFromEnv
    ? '<span class="statusOk">Using <code>VERIFY_TOKEN</code> from <code>.env</code> (value not shown)</span>'
    : '<span class="statusWarn">Using the default token from <code>server.js</code>. Set <code>VERIFY_TOKEN</code> in <code>.env</code> for production.</span>';

  return `<section class="section" aria-label="Settings">
        <div class="sectionHead">
          <div>
            <div class="sectionTitle">Settings</div>
            <div class="sectionHint">WhatsApp connection and public restaurant details</div>
          </div>
          <a class="history" href="/"><span>Overview</span> <span aria-hidden="true">→</span></a>
        </div>
        ...
        <div class="setGrid">
          <div class="card setCard">
            ...
            <p class="setCardIntro">Values come from your host environment (<code>.env</code>). Secrets are never displayed on this page.</p>
            <div class="setRow">
              <div class="setLabel">Phone number ID</div>
              <div class="setVal">${idRow}</div>
            </div>
            <div class="setRow">
              <div class="setLabel">Access token</div>
              <div class="setVal">${tokRow}</div>
            </div>
            <div class="setRow">
              <div class="setLabel">Webhook verify</div>
              <div class="setVal">${verifyRow}</div>
            </div>
            <div class="setRow">
              <div class="setLabel">Callback path</div>
              <div class="setVal">Configure Meta webhook URL as <code>https://&lt;your-public-host&gt;/webhook</code> with the same verify token.</div>
            </div>
          </div>

          <div class="card setCard">
            <h2>Restaurant profile</h2>
            <p class="setCardIntro">Shown to customers in WhatsApp. Edit the <code>R</code> object in <code>server.js</code> and redeploy.</p>
            <div class="setRow">
              <div class="setLabel">Name</div>
              <div class="setVal">${escapeHtml(R.name)}</div>
            </div>
            <div class="setRow">
              <div class="setLabel">Address</div>
              <div class="setVal">${escapeHtml(R.address)}</div>
            </div>
            <div class="setRow">
              <div class="setLabel">Hours</div>
              <div class="setVal">${escapeHtml(R.timing)}</div>
            </div>
            <div class="setRow">
              <div class="setLabel">Min order</div>
              <div class="setVal">${escapeHtml(String(R.min_order))} (${escapeHtml(R.currency)})</div>
            </div>
            <div class="setRow">
              <div class="setLabel">Delivery</div>
              <div class="setVal">${escapeHtml(R.delivery_time)}</div>
            </div>
          </div>
        </div>
      </section>`;
...
/**
 * @param {{ R: object, PHONE_NUMBER_ID?: string, orders: object[], page: 'overview'|'orders'|'menu'|'settings', settingsMeta?: { hasAccessToken?: boolean, verifyFromEnv?: boolean } }} opts
 */
function renderAdminPage(opts) {
  const { R, PHONE_NUMBER_ID, orders, page, settingsMeta = {} } = opts;
