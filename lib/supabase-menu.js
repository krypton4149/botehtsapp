...
/**
 * Menu for the WhatsApp bot: if `fallbackMenu` (e.g. R.menu from server.js) has categories with items,
 * that embedded menu is used first. Otherwise loads from Supabase with a short TTL cache.
 * @param {Record<string, Array<{ id: string; name: string; price: number; veg: boolean }>>} fallbackMenu
 * @returns {Promise<{ menu: Record<string, any[]>, source: 'embedded' | 'db' | 'fallback' }>}
 */
async function getBotMenuRecord(fallbackMenu) {
  ...
  const embedded =
    fallbackMenu && typeof fallbackMenu === 'object' && !Array.isArray(fallbackMenu) ? fallbackMenu : {};
  const embeddedMenu = {};
  for (const [title, rows] of Object.entries(embedded)) {
    if (Array.isArray(rows) && rows.length) embeddedMenu[title] = rows;
  }
  if (Object.keys(embeddedMenu).length > 0) {
    botMenuCache = {
      menu: embeddedMenu,
      expiresAt: Date.now() + BOT_MENU_CACHE_TTL_MS,
      source: 'embedded',
    };
    return { menu: embeddedMenu, source: 'embedded' };
  }
