...
const {
  ensureOrdersOutForDeliveryColumn,
  isMissingOutForDeliveryColumnError,
} = require('./ensure-orders-dispatch-column');
...
async function fetchOrdersForDashboard() {
  ...
  // Use * so older DBs without out_for_delivery still load (explicit column list errors if column missing).
...
async function setOrderOutForDelivery(orderNum, outForDelivery) {
  ...
  if (error && isMissingOutForDeliveryColumnError(error)) {
    const fixed = await ensureOrdersOutForDeliveryColumn();
    if (fixed) {
  ...
  if (error) {
    ...
    if (isMissingOutForDeliveryColumnError(error)) {
      return {
        ok: false,
        code: 503,
        message:
          'Database column out_for_delivery is missing. In Supabase → SQL Editor, run:\n\n' +
          'ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS out_for_delivery boolean NOT NULL DEFAULT false;\n\n' +
          '(Or set SUPABASE_DB_URL with your direct Postgres connection string so the server can add it automatically.)',
      };
    }
