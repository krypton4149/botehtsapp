...
async function fetchOrdersForDashboard() {
  ...
  const { data: ords, error } = await supabase
    .from('orders')
    .select(
      'id, order_num, customer_name, phone, whatsapp, address, total, status, currency, placed_at'
    )
...
async function insertOrderFromCheckout(payload) {
  ...
  const { data: ord, error: oErr } = await supabase
    ...
    .select('id, order_num, customer_name, phone, whatsapp, address, total, status, currency, placed_at')
    .single();
